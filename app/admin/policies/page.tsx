...
function ToggleRole({ role, selected, onChange, disabled }: { role: Role; selected: boolean; onChange: (v: boolean) => void; disabled: boolean }) {
  return (
    <label className={`inline-flex cursor-pointer items-center gap-2 rounded-md border px-3 py-1 text-sm ${selected ? 'border-primary bg-primary/10' : 'border-input'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
      <input
        type="checkbox"
...
function PolicyForm({ resourceId, initial, onSave, onCancel, disabled }: { resourceId: string; initial?: AccessPolicy; onSave: (p: AccessPolicy) => void; onCancel: () => void; disabled: boolean }) {
  const [resourceIdValue, setResourceIdValue] = useState(resourceId)
  const [minTier, setMinTier] = useState<MembershipTier | undefined>(initial?.minTier)
  ...
  const handleSubmit = () => {
    const policy: AccessPolicy = { resourceId: resourceIdValue, minTier, roles: roles.length > 0 ? roles : undefined }
  ...
  return (
    <div className="space-y-3 rounded-md border p-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Resource ID</label>
          <Input
          ...
          />
          {errors.resourceId && <p className="mt-1 text-xs text-destructive">{errors.resourceId}</p>}
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Minimum Tier</label>
          <select
            className="flex h-9 w-full rounded-md border border-input bg-transparent px-2 text-sm"
            value={minTier ?? ''}
            onChange={(e) => setMinTier((e.target.value || undefined) as MembershipTier | undefined)}
            disabled={disabled}
          >
          ...
          {errors.minTier && <p className="mt-1 text-xs text-destructive">{errors.minTier}</p>}
        </div>
      </div>

      <div>
        <label className="mb-1 block text-xs font-medium text-muted-foreground">Required Roles</label>
      ...
      {errors.combination && (
        <p className="text-xs text-destructive" role="alert">{errors.combination}</p>
      )}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
              variant="outline"
              onClick={() => { setShowCreateForm(!showCreateForm); setEditingResourceId(null) }}
            >
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {isLoading ? (
                ...
                <EmptyState title="No resources configured" message="No access policies have been configured yet. Create one above to get started." />
              ) : (
                <>
                  {/* Show resources with their policies */}
                  {(resources ?? []).map((resource) => {
                    const resourcePolicies = policies?.filter((p) => p.resourceId === resource.id) ?? []
                    const isEditing = editingResourceId === resource.id

                    return (
                      <div key={resource.id} className="space-y-2 rounded-md border p-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-sm">{resource.title || resource.id}</span>
                              {resourcePolicies.length > 0 && (
                                <Badge variant="secondary">{resourcePolicies.length} policy{resourcePolicies.length > 1 ? 's' : ''}</Badge>
                              )}
                            </div>
                          ...
                          <Button
                            ...
                            size="sm"
                            variant="ghost"
                            onClick={() => { setEditingResourceId(isEditing ? null : resource.id); setShowCreateForm(false) }}
                            disabled={Boolean(pendingPolicyId)}
                          >
                                  ...
                                  {errors && Object.keys(errors).length > 0 && (
                                    <div className="text-xs text-destructive" role="alert">
                                      {Object.values(errors).filter(Boolean).map((msg) => (
                                        <span key={msg} className="block">{msg}</span>
                                      ))}
                                    </div>
                                  )}
                                </div>
                  ...
                  {policies && policies.length > 0 && resources && resources.length > 0 && (() => {
                    const resourceIds = new Set(resources.map((r) => r.id))
                    const orphaned = policies.filter((p) => !resourceIds.has(p.resourceId))
                    if (orphaned.length === 0) return null
                    return (
                      <div className="space-y-2">
                        <p className="text-xs font-medium text-muted-foreground">Unassigned Policies</p>
                        {orphaned.map((policy) => {
                          const errors = formErrors[policy.resourceId]
                          return (
                            <div key={policy.resourceId} className="rounded-md border p-3 space-y-1">
                              <div className="flex items-center gap-2 text-sm">
                                <span className="font-mono text-xs">{policy.resourceId}</span>
                                <Badge variant="outline">{policy.minTier ?? 'free'}</Badge>
                                {policy.roles && policy.roles.length > 0 && policy.roles.map((r) => (
                                  <Badge key={r} variant="secondary">{r}</Badge>
                                ))}
                                {pendingPolicyId === policy.resourceId && (
                                  <Badge variant="warning">Saving</Badge>
